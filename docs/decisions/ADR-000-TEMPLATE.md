...
> Copy this file to `ADR-NNN-short-kebab-title.md`, take the next free `NNN`, and delete this
...
**Relates to:** the design doc under `../designs/`, the glossary terms it defines, and the
...
- **What stays open.** Questions this does not answer. Do not let an ADR imply closure it
  did not achieve.
- **What now depends on it.** Code, docs, or designs that would break if this were revisited.
...
- measurements, with the number, how the run was parameterised, and what was warmed
...
out to the transcript or source this argument came from.
